...
set.seed(12345)
setwd("~/Library/CloudStorage/OneDrive-KULeuven/PhD Garin/new Michiel_combinational_therapy_analysis_2023")

## define a function to extract the data
extract_data <- function(filename,output_folder) {
  NAME = word(filename,1)
  bio_rep = extract_numeric(word(filename,4))
  output_name = word(filename, 1, sep=" - ")
  #################    slice datasets by celline and OA concentration    ####
  # EGFR cellline
  data1 <- read_excel(path=filename, sheet = "EGFP", range = "N11:W14"  , na="") #OA=10nM
  data2 <- read_excel(path=filename, sheet = "EGFP", range = "N16:W19"  , na="")
 
  #WT cellline
  data3 <- read_excel(path=filename, sheet = "WT", range = "N11:W14"  , na="") #OA=10nM
  data4 <- read_excel(path=filename, sheet = "WT", range = "N16:W19"  , na="")
  
  #mutant celline R183W
  data5 <- read_excel(path=filename, sheet = "R183W", range = "N11:W14"  , na="") #OA=10nM
  data6 <- read_excel(path=filename, sheet = "R183W", range = "N16:W19"  , na="")
  
  #P179R celline 
  data7 <- read_excel(path=filename, sheet = "P179R", range = "N11:W14"  , na="") #OA=10nM
  data8 <- read_excel(path=filename, sheet = "P179R", range = "N16:W19"  , na="")
  
  #S256F celline
  data9 <- read_excel(path=filename, sheet = "S256F", range = "N11:W14"  , na="") #OA=10nM
  data10 <- read_excel(path=filename, sheet = "S256F", range = "N16:W19"  , na="")

  #################    assembly EGFR cell_line data   ############################
  data1 <- data1  %>%  gather(key="DRUG_CONC",value="DV", -"OA 10nM") %>% 
    mutate(REP=extract_numeric(`OA 10nM`),
           ADDITIVE_CONC=10,
           CELLINE=1,
           CELLINE_NAME="EGFP",
           DRUG_NAME=NAME,
    ...
    select(CELLINE,CELLINE_NAME,DRUG_NAME,ADDITIVE_CONC,REPEAT,REP,DRUG_CONC,DV,-`OA 10nM`) %>% arrange(REP,DRUG_CONC)
  
  data2 <- data2  %>%  gather(key="DRUG_CONC",value="DV", -"OA 0nM") %>% 
    mutate(REP=extract_numeric(`OA 0nM`),                                # here `OA 0nM` is a column, not a string
           ADDITIVE_CONC=0,
           CELLINE=1,
           CELLINE_NAME="EGFP",
           DRUG_NAME=NAME,
           REPEAT=bio_rep,
           DRUG_CONC=ifelse(DRUG_CONC=="DMSO",0,as.numeric(DRUG_CONC))) %>% 
    select(CELLINE,CELLINE_NAME,DRUG_NAME,ADDITIVE_CONC,REPEAT,REP,DRUG_CONC,DV,-`OA 0nM`) %>% arrange(REP,DRUG_CONC)
  
  
  ##################    assembly WT cell_line data   ############################
  data3 <- data3  %>%  gather(key="DRUG_CONC",value="DV", -"OA 10nM") %>% 
    mutate(REP=extract_numeric(`OA 10nM`),
           ADDITIVE_CONC=10,
           CELLINE=2,
           CELLINE_NAME="WT",
  ...
  data4 <- data4  %>%  gather(key="DRUG_CONC",value="DV", -"OA 0nM") %>% 
    mutate(REP=extract_numeric(`OA 0nM`),                                # here `OA 0nM` is a column, not a string
           ADDITIVE_CONC=0,
           CELLINE=2,
           CELLINE_NAME="WT",
  ...
  ##################    assembly mutant R183W cell_line data   ########################
  data5 <- data5  %>%  gather(key="DRUG_CONC",value="DV", -"OA 10nM") %>% 
    mutate(REP=extract_numeric(`OA 10nM`),
           ADDITIVE_CONC=10,
           CELLINE=3,
           CELLINE_NAME="R183W",
  ...
  data6 <- data6  %>%  gather(key="DRUG_CONC",value="DV", -"OA 0nM") %>% 
    mutate(REP=extract_numeric(`OA 0nM`),                                # here `OA 0nM` is a column, not a string
           ADDITIVE_CONC=0,
           CELLINE=3,
           CELLINE_NAME="R183W",
  ...
  ##################    assembly P179R cell_line data   ########################
  data7 <- data7  %>%  gather(key="DRUG_CONC",value="DV", -"OA 10nM") %>% 
    mutate(REP=extract_numeric(`OA 10nM`),
           ADDITIVE_CONC=10,
           CELLINE=4,
           CELLINE_NAME="P179R",
  ...
  data8 <- data8  %>%  gather(key="DRUG_CONC",value="DV", -"OA 0nM") %>% 
    mutate(REP=extract_numeric(`OA 0nM`),                                # here `OA 0nM` is a column, not a string
           ADDITIVE_CONC=0,
           CELLINE=4,
           CELLINE_NAME="P179R",
  ...
  ##################    assembly S256F cell_line data   ########################
  data9 <- data9  %>%  gather(key="DRUG_CONC",value="DV", -"OA 10nM") %>% 
    mutate(REP=extract_numeric(`OA 10nM`),
           ADDITIVE_CONC=10,
           CELLINE=5,
           CELLINE_NAME="S256F",
  ...
  data10 <- data10  %>%  gather(key="DRUG_CONC",value="DV", -"OA 0nM") %>% 
    mutate(REP=extract_numeric(`OA 0nM`),                                # here `OA 0nM` is a column, not a string
           ADDITIVE_CONC=0,
           CELLINE=5,
           CELLINE_NAME="S256F",
  ...
  ##################   bind data 1-10 together   ##################################
  data <-  bind_rows(data1, data2, data3,data4,data5,data6,data7,data8,data9,data10)
  write.csv(data, paste0(output_folder,output_name,".csv"), quote=F, row.names=F)
}

## generate the extracted datasets
path  <- "~/Library/CloudStorage/OneDrive-KULeuven/PhD Garin/new Michiel_combinational_therapy_analysis_2023/data assembly/"
file_info <- data.frame(filename = list.files(getwd(),pattern = "\\.xlsx$"),
                        output_folder=path)

purrr::pmap(list(file_info$filename,file_info$output_folder), extract_data)

## combine all datasets
files <- list.files(path,pattern = "\\.csv$")
combined_data <- data.frame()
for (i in files){
  data <- read.csv(file = paste0(path,i))
  combined_data <- bind_rows(combined_data,data)
}
###################generate final dataset for modeling   #######################
DS_2023 <- combined_data %>% filter(!is.na(DV))%>% 
  mutate(ADDITIVE_NAME="OA", 
         ADDITIVE=1,
         DV=DV*0.01,
...
DS_2023 <- DS_2023 %>% group_by(DRUG_NAME)                                       %>% mutate(DRUG                                        = cur_group_id()) %>% ungroup() 
DS_2023 <- DS_2023 %>% group_by(CELLINE, DRUG, ADDITIVE)                         %>% mutate(CELLINE_DRUG_ADDITIVE                       = cur_group_id()) %>% ungroup() # ID1
DS_2023 <- DS_2023 %>% group_by(CELLINE, DRUG, ADDITIVE, ADDITIVE_CONC)          %>% mutate(CELLINE_DRUG_ADDITIVE_ADDITIVECONC          = cur_group_id()) %>% ungroup() # ID2
DS_2023 <- DS_2023 %>% group_by(CELLINE, DRUG, ADDITIVE, ADDITIVE_CONC,REPEAT)   %>% mutate(CELLINE_DRUG_ADDITIVE_ADDITIVECONC_REPEAT   = cur_group_id()) %>% ungroup() # ID3

## rearrange the data and export the dataset
DS_2023 <- DS_2023 %>% select(CELLINE,DRUG,ADDITIVE,ADDITIVE_CONC,REPEAT,REP,DRUG_CONC,DV,CELLINE_DRUG_ADDITIVE,
                    CELLINE_DRUG_ADDITIVE_ADDITIVECONC,CELLINE_DRUG_ADDITIVE_ADDITIVECONC_REPEAT,CELLINE_NAME,DRUG_NAME,ADDITIVE_NAME,CELLINE_DRUG_ADDITIVE_NAME) 
#write.csv(DS_2023, paste0(path,"DS_2023_final.csv"), quote=F, row.names=F)
#####################  END OF DATA MANAGEMENT  #################################
################################################################################

##########################  DATA VISUALISATION  #################################
data_plot <- DS_2023 %>% mutate(DRUG_CONC = ifelse(DRUG_CONC == 0, 0.0001, DRUG_CONC),
...
# 1 Clofarabine - OA ############
p1 <- ggplot() +
  geom_point(data = DS_2023 %>% filter(CELLINE_NAME == "EGFP" & DRUG_NAME == "Clofarabine" & ADDITIVE_NAME == "OA") %>% mutate(DRUG_CONC = ifelse(DRUG_CONC == 0, 0.0001, DRUG_CONC)),
...
p2 <- ggplot() +
  geom_point(data = DS_2023 %>% filter(CELLINE_NAME == "WT" & DRUG_NAME == "Clofarabine" & ADDITIVE_NAME == "OA") %>% mutate(DRUG_CONC = ifelse(DRUG_CONC == 0, 0.0001, DRUG_CONC)),
             mapping = aes(x = DRUG_CONC, y = DV, colour = as.factor(ADDITIVE_CONC)), alpha = 0.2) +  # ADDITIVECONC is a covariate
  # scale_x_log10(breaks = c(0, 0.001,0.025, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  ),
  #               labels = scales::number_format(accuracy = 0.001)) +
  scale_x_continuous(breaks = c(0,0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  )) +
...
p3 <- ggplot() +
  geom_point(data = DS_2023 %>% filter(CELLINE_NAME == "R183W" & DRUG_NAME == "Clofarabine" & ADDITIVE_NAME == "OA") %>% mutate(DRUG_CONC = ifelse(DRUG_CONC == 0, 0.0001, DRUG_CONC)),
             mapping = aes(x = DRUG_CONC, y = DV, colour = as.factor(ADDITIVE_CONC)), alpha = 0.2) +  # ADDITIVECONC is a covariate
  # scale_x_log10(breaks = c(0, 0.001,0.025, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  ),
  #               labels = scales::number_format(accuracy = 0.001)) +
  scale_x_continuous(breaks = c(0,0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  )) +
...
p4 <- ggplot() +
  geom_point(data = DS_2023 %>% filter(CELLINE_NAME == "P179R" & DRUG_NAME == "Clofarabine" & ADDITIVE_NAME == "OA") %>% mutate(DRUG_CONC = ifelse(DRUG_CONC == 0, 0.0001, DRUG_CONC)),
             mapping = aes(x = DRUG_CONC, y = DV, colour = as.factor(ADDITIVE_CONC)), alpha = 0.2) +  # ADDITIVECONC is a covariate
  # scale_x_log10(breaks = c(0, 0.001,0.025, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  ),
  #               labels = scales::number_format(accuracy = 0.001)) +
  scale_x_continuous(breaks = c(0, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  )) +
...
p5 <- ggplot() +
  geom_point(data = DS_2023 %>% filter(CELLINE_NAME == "S256F" & DRUG_NAME == "Clofarabine" & ADDITIVE_NAME == "OA") %>% mutate(DRUG_CONC = ifelse(DRUG_CONC == 0, 0.0001, DRUG_CONC)),
             mapping = aes(x = DRUG_CONC, y = DV, colour = as.factor(ADDITIVE_CONC)), alpha = 0.2) +  # ADDITIVECONC is a covariate
  # scale_x_log10(breaks = c(0, 0.001,0.025, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1  ),
  #               labels = scales::number_format(accuracy = 0.001)) +
  scale_x_continuous(breaks = c(0, 0.05, 0.1, 0.2, 0.4, 0.6, 0.8, 1)) +
...
DS_dat <- full_join(DS_2023 %>%  select(ID=CELLINE_DRUG_ADDITIVE_ADDITIVECONC, CELLINE_DRUG_ADDITIVE,CELLINE, DRUG, ADDITIVE, ADDITIVE_CONC, CELLINE_NAME, DRUG_NAME, ADDITIVE_NAME, CELLINE_DRUG_ADDITIVE_NAME) %>% unique(),
...
dat_trapezoid <- cbind(DSS, DS_2023 %>% select(DRUG_CONC,DV,CELLINE_DRUG_ADDITIVE_ADDITIVECONC_REPEAT,REP))
...
#                                 DSS3i = DSS2i *((upper_DRUGCONC)/(upper_DRUGCONC))) 
# 
# DS_dat <- full_join(DS_2023 %>%  select(ID_DAY=CELLINE_DRUG_ADDITIVE_ADDITIVECONC_REPEAT, ID = CELLINE_DRUG_ADDITIVE_ADDITIVECONC, CELLINE_DRUG_ADDITIVE,CELLINE, DRUG, ADDITIVE, ADDITIVE_CONC, DAY=REPEAT, CELLINE_NAME, DRUG_NAME, ADDITIVE_NAME, CELLINE_DRUG_ADDITIVE_NAME) %>% unique() %>%
